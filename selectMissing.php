...
<main>
    <h1 class="quiz-title text-center heading">Select Missing Word</h1>
    <div class="container mt-5">
      <div class = "Instructions">
        <h1>Instructions:</h1>
        <ol>
        <li>You will hear a recording about fiction writing. At the end of the recording the lost word or group of words has been replaced by a beep. Select the correct option to complete the recording.</li>
        </ol>
      </div>
    <h3>Test <span id = "index">1</span></h3>
        ...
        <source src="audio1/question1.mp3" type="audio/mpeg">
...
xhr.open("GET", "selectMissing.json", true);
xhr.onreadystatechange = function () {
  if (xhr.readyState === 4) {
    if (xhr.status === 200) {
      ...
      console.error('Failed to load questions. HTTP status:', xhr.status);
    }
  }
};
xhr.send();
...
function loadQuestion(index) {
  ...
  for (let i = 0; i < question.options.length; i++) {
    optionsHtml += `<div class="form-check">
                       <input class="form-check-input" type="radio" name="q${index}" id="q${index}a${i}" value="${question.options[i]}">
                       <label class="form-check-label" for="q${index}a${i}">${question.options[i]}</label>
                     </div>`;
  }
  optionsContainer.innerHTML = optionsHtml;
  audio.src = `audio1/question${currentQuestion + 1}.mp3`;
    resultContainer.innerHTML = ''; // Clear feedback
}
