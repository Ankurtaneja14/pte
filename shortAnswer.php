...
<main>
    <h1 class="mb-4 heading">Short Answer Question Test</h1>
<div class="container mt-5">
<div class = "Instructions">
      <h1>Instructions:</h1>
      <ol>
      <li>You will hear a question. Please give a simple and short answer. Often just one or a few words is enough.</li>
      </ol>
    </div>
        <div class="row">
          ...
          <p>Total Tests: 331</p>
            <div class="col-md-12 buttons">
                <button id="readAloudButton" class="btn btn-success" hidden>Listen Sentence</button>
                <button id="startRecordingButton" class="btn btn-success" disabled>Start Recording</button>
            ...
            <div id = "Answer" class="card mt-4">
            <p>Question: <span id="practiceParagraph"></span></p>
            <p>Answer: <span id ="practiceAnswer"></span></p>
    ...
    <h3>AI Score</h3>
    <div id="scoreDetails">
      <p>User Speech: <span id="userSpeech"></span></p>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const userSpeechElement = document.getElementById("userSpeech"); // Added userSpeechElement
    let timerInterval;
    ...
    const userSpeech = phrases[0];
    console.log("User's speech: " + userSpeech); // Check if user's speech is captured
    console.log("userSpeechElement: ", userSpeechElement); // Check if the element exists
    userSpeechElement.textContent = userSpeech
    ...
    function updateParagraph() {
        if (paragraphs) {
            const currentParagraph = paragraphs[currentParagraphIndex];
            practiceParagraph.textContent = currentParagraph.title;
            practiceAnswer.textContent = currentParagraph.content;
        }
    }

    readAloudButton.addEventListener("click", function () {
        const currentParagraph = paragraphs[currentParagraphIndex];
        const utterance = new SpeechSynthesisUtterance(currentParagraph.title);
    ...
    if (paragraphs) {
        const currentParagraph = paragraphs[currentParagraphIndex];
        const userSpeech = userSpeechElement.textContent;
        const score = userSpeech.toLowerCase() === currentParagraph.content.toLowerCase() ? "1/1" : "0/1";
        userSpeechElement.textContent = ""; // Clear user's speech

        // Display the calculated score
        document.getElementById("totalScore").textContent = score;
        userSpeechElement.textContent = userSpeech
        // You can also perform other actions based on the score.
    }
    aiScore.style.display = "block";
    submitButton.disabled = true; // Disable the Submit button after calculating the score
});
